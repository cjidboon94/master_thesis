...
with mp.Pool(os.cpu_count()) as pool:
	results = pool.map(real_experiment, [(n_var, "sis", beta, dists, interventions, seeds[i]) for i, (beta, n_var) in enumerate(itertools.product(betas, n_vars))]) #callback=callback)
callback(results)
