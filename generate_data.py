...
class Circle(Grid):
    ...
    def initialize(self, center, radius):
        up, right, down, left = (
            min(int(center[0]+(radius)), self.grid_length-1), 
            min(int(center[1]+(radius)), self.grid_width-1),
            max(int(center[0]-(radius)), 0),
            max(int(center[1]-(radius)), 0),
        )
        print("up {}, right {}, down {}, left {}".format(up, right, down, left))
        for i in range(down, up+1):
            for j in range(left, right+1):
                #print(self.distance([i, j], self.center))
                if self.distance([i, j], self.center) <=  self.radius:
                    self.grid[i, j] = 1     

    def generate_random(self, radius_ran, distance_border):
...
class Rectangle(Grid):
    ...
    def generate_random(distance_border, width_ran, length_range, rotate=True):
        center = self.get_random_point_in_grid(distance_border)
        ...
        self.initialize(center, width, length, rotation)        
        return np.flatten(np.copy(self.grid))
